...
public partial class auth_add_delivery_boy : System.Web.UI.Page
{
    ...
    {
        SqlDataReader readiddr = mst.Select_Operation("SELECT Max(id) as id FROM student ");
        if (readiddr.Read())
        {
            ...
            {
                lbl_id.Text = "STD-" + Convert.ToString(Convert.ToInt32(readiddr["id"].ToString()) + 1);
            }
        }
        else
        {
    ...
    protected void btnsaveAndnext_ServerClick(object sender, EventArgs e)
    {
        try
        {
            if(txt_address_line_1.Text.Length>0 && txt_mobileno.Text.Length>0 && txt_name.Text.Length>0 && txt_father.Text.Length>0 && txtx_mother.Text.Length>0 && txt_pincode.Text.Length>0 && dbl_state.SelectedItem.Text!="Please Select" && dbl_city.SelectedItem.Text!="Please Select")
            {
                GenerateId();

                int success = bnc.Add_Student(lbl_id.Text, txt_name.Text, txt_mobileno.Text,txt_father.Text, txtx_mother.Text, txt_address_line_1.Text,  dbl_state.SelectedItem.Text,  dbl_city.SelectedItem.Text, txt_pincode.Text, DateTime.Now.ToString("yyyy-MM-dd"), DateTime.Now.ToString("hh:mm tt"), dbl_status.SelectedValue, dblclass.SelectedItem.Text, dblsection.SelectedItem.Text);
